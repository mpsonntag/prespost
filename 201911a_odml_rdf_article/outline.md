...
After reviewing the general structure of odML entities there were two main points to consider: odML entities (Document, Section, Property) and the entities' Attributes (e.g. Section.name or Property.value). To approach the issue of converting the hierarchical structure to a graph based representation, as a general concept, odML entities where considered as Graph nodes while odML entity attributes where considered graph leaves. In this Scenario mapping entity attributes to RDF was straightforward. As an example, the odML attribute `Section.name="Experiment_A"` is converted straightforward to the RDF triple `odml:Section odml:hasName Experiment_A`.

Mapping the connection between entities was another issue. 
Uniquely identifying an entity and its connections within a graph is essential to maintain and reconstruct an underlying hierarchical structure. RDF provides the feature of "blank nodes" to uniquely identify a graph node within an RDF graph and provides it out of the box, if no unique identifier for an RDF node has been provided. The downside of blank nodes in RDF are memory transient. As an example, if the same RDF file is loaded into two different graphs, the IDs of the same blank nodes will receive different IDs in these two graphs. This does keep the underlying structure intact, but makes identification of a specific node problematic. Furthermore, if the same RDF file is uploaded again into the same graph, the blank node IDs differ again, leading to duplicate entries in the graph.
odML already provides all its main entities (Document, Section, Property) with its own unique IDs, that uniquely identifies them within and even across different odML documents. These IDs where now used as identifiers for the all nodes representing odML entities. With this approach, loading an odML RDF file into different graphs or even into the same graph twice, the odML entities are always uniquely identifiable and duplicate entries do not occur.

The abstract example in Figure[TODO] shows the problemset and the chosen approach. To be specific, the odML entities' unique IDs are UUID strings as specified in RFC 4122\footnote{\url{https://tools.ietf.org/html/rfc4122}} with an extremely low collision probability.
