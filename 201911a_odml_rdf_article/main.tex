...
%[JG] but harder to master?
%[MS] That SPARQL is hard to master is partially addressed in the usage section and it is mentioned in the outlook that we are aware of the aspect and have a prototype to build a more easy to use query layer on top of SPARQL.
\textit{SPARQL}\footnote{\url{https://www.w3.org/TR/sparql11-query}} query language.

%[JG] Do you want to provide a very brief intro to RDF? Similar to what you do for odml?
...
%[TW] odML terminologies should be introduced
%[MS] added a paragraph before the file format description paragraph above to not break the reading flow at this point. Can be changed if it works better at another location.
...
%[MS] Added an "addressing conceptual issues" section below to address the comment
...
In the Semantic Web the global identifier is called \texttt{namespace IRI}\footnote{\url{https://www.w3.org/TR/rdf-syntax-grammar\#section-Namespace}} (Internationalised Resource Identifier) and is used to distinguish entities from different Datasets. The IRI of RDF for example is \texttt{http://www.w3.org/1999/02/22-rdf-syntax-ns\#}. For odML RDF, the namespace "https://g-node.org/odml-rdf" was chosen to identify all RDF odML entities.

Further conceptual issues to consider on translating the odML format to RDF were the connection of multiple, stand alone documents into a single graph, keeping odML entities uniquely identified, avoiding duplicate entries and respecting sequence order.

To allow the import of multiple, unrelated odML documents into a single RDF graph, we introduced an RDFS class called "odml:Hub". When an odML document is exported to RDF, the "odml:Document" RDF object always gains an "odml:Hub" with always the same identifier "Hub" as its parent node. Using this approach, when importing multiple documents into a single graph, all "odml:Documents" will have the same "odml:Hub" node as its parent, linking them together and rooting the graph at the "Hub" node.

odML features unique ID strings for each of its entities (Document, Section and Property). These are defined as RFC 4122\footnote{\url{https://datatracker.ietf.org/doc/html/rfc4122}} compliant UUIDs which make these IDs for practical purposes globally unique, ensuring, that even across odML documents same-name entities can still be uniquely identified.
In RDF on the other hand subjects and objects do not necessarily have to be assigned fixed identifiers but can be defined as "blank nodes". When objects defined as blank nodes are loaded into an RDF graph, these nodes will receive an internal identifier  for the purpose of querying. Issues can arise, if the same RDF file is imported into the same graph twice: nodes defined as blank nodes will receive new blank node IDs and the associated triples will be imported as duplicates.
...
odML Properties can store multiple values and keep the order of these values intact when loading from or saving to file. RDF by default does not respect order; when loading an RDF file into a graph and saving this graph to a new file, all associated triples saved will be identical, but the order in which they appear will be different between the two files.
odML Property values should be searchable individually and should not be exported as e.g. a single string concatenation of these values. Further the order of these values should still be available after the export to RDF. Connecting each individual value to its property by using e.g. "odml:hasValue" predicates would not respect the value order; each value would be saved as an equal triple.
RDF does provide a Sequence class (rdf:Seq) featuring all child nodes connected by numbered predicates to its parent node, allowing to retrieve the child nodes order if necessary. To this end, we export odML Property values as numbered rdf:Seq children and connect the rdf:Seq object via an "odml:hasValue" predicate to the odml:Property object to retain the order.

\subsubsection{Mapping odML entities to RDF classes} \label{sec:odml_rdf_mapping}
...
For the actual mapping, the odML entities \texttt{Document}, \texttt{Section} and \texttt{Property} where mapped to RDF classes, while all attributes of the individual odML entities, e.g. \texttt{name} or \texttt{reference} where mapped to RDF Literals. An exception are the \texttt{Property.values} that require a container element, the \texttt{RDF.Seq}. As previously discussed this is necessary since an RDF graph does not retain any order between its contained triples. \texttt{RDF.Seq} is a class that allows to specify an order of the contained elements. Only this allows an exact export of odML values to RDF, where the order of values often is of import, for example in the case of documenting a set of stimuli. \cref{table:Document,table:Section,table:Property,table:Hub,table:Terminology} describe the mapping in detail.
...
\begin{tablenotes}
\small
\item Mapping of odML \texttt{Document} to RDF. (a) The document root is mapped to an RDFS class of type \texttt{odml:Document}. (b) The UUID of the odML \texttt{Document} is used as Name of the created \texttt{odml:Document} RDF node to uniquely identify the document when it is merged with a graph database. (c) RDF export of repositories: linked terminologies are downloaded on conversion and exported additional RDF document connected via the \texttt{odml:Hub} RDF node. This additional document is linked via \texttt{odml:hasTerminology} to the original document. If the terminology cannot be imported, its URL will be kept as is and its value is provided as an RDF leaf via the predicate \texttt{odml:hasExternalTerminology} in the original document. See ~\ref{table:Hub} and ~\ref{table:Terminology} for more detailed descriptions.
(d) As provenance of where the RDF odml:Document was created from, the filename of the original odML file is documented via the non-odML format predicate \texttt{odml:hasFilename}.
\end{tablenotes}
...
\begin{tablenotes}
\item Mapping of odML \texttt{Property} to RDF. (a) Any odML \texttt{Property} entity is mapped to an RDFS class of type \texttt{odml:Property}. (b) The UUID of an odML \texttt{Property} entity is used as Name of the created \texttt{odml:Property} RDF node to uniquely identify the section when it is merged with a graph database. (c) cf. \texttt{reference} description in Table ~\ref{table:Section}. (d) odML supports multiple values. Upon export to RDF the order of these values need to be respected as well. Values are connected to the RDF document via the \texttt{odml:hasValue} predicate to an \texttt{rdf:Seq} node. This \texttt{rdf:Seq} node contains numbered \texttt{rdf:li} items which in turn contain the individual \texttt{odml.Value entries}. This construct enables RDF searches for individual values rather then searching for lists of values.
\end{tablenotes}
...
\caption{Hub}
\begin{tablenotes}
\item The custom RDF type of class \texttt{odml:Hub} has no odML entity equivalent. It is introduced to merge Documents and Terminologies into a single RDF graph: the "Hub" (also using "Hub" as the RDF node instance Name) is used to root a graph that might contain multiple documents enabling searches across unrelated and heterogeneous metadata content.
\end{tablenotes}
\label{table:Hub}
\end{threeparttable}
\end{table}
...
All of the odml RDFS classes can be fine grained into subclasses to give more semantic meaning to the connection between nodes. For example \texttt{section rdf:type odml:Section} becomes \texttt{section rdf:type odml:Experimenter}. Here \texttt{odml:Experimenter} is a subclass of \texttt{odml:Section}. Using our current example, a search for \texttt{odml:Experimenter} returns only sections of this specific class, while a search for \texttt{odml:Section UNION rdfs:subClassOf odml:Section} returns all pure and subclassed sections including odml:Experimenter.

\subsubsection{odML RDF ontology} \label{sec:odml_ontology}
To make use of the RDF feature of validating individual RDF files, we created an odML RDF ontology which includes the basic set of odML RDFS terms and included all currently supported odml:Section subclasses. The complete file can be found in the Appendix under Listing ~\ref{lst:ontology}. This list of subclasses should not be seen as permanent and is expected to expand in the future.
...
$ odmlimportdatacite [-f FORMAT] [-o OUT] [-n NAMESPACE ...] [-r] [-p] INPUT
\end{lstlisting}
...
\texttt{-n NAMESPACE} An optional argument. By default only the namespace 'http://datacite.org/schema/kernel-4' is properly handled. Use this option to add any additional namespaces DataCite files may contain so they can be properly parsed; this might be the case when converting previous DataCite file versions. The option can be used multiple times.
...
An existing odML file which was used to document a published set of experiments can easily be combined with an odML file created from a DataCite XML file and exported to RDF to search for both values from the experiment or publication information and arrive at a link to the published data set.
...
# Paths of two example odML RDF files that will be loaded
...
This query will return DOI links to the data sets annotated with Keywords "Neuroscience" or "Electrophysiology". As additional information it also provides the filename of the metadata file the data originated from, the type of Identifier, and the matching keyword. Likewise a search could start from searching for specific authors of a data set. The results of the query in ~\ref{lst:query_graph} can be found in ~\ref{lst:query_result}.
...
Fuseki is an open source SPARQL query server from the \textit{Apache Jena}\footnote{\url{https://jena.apache.org} (Jena 2006, A semantic web framework for Java)} Semantic Web tools suite that was adopted to support odML specific RDF. A publicly accessible instance is available at https://meta.g-node.org. The service provides a metadata graph queryable by SPARQL containing all scientific Datasets that have been published by \textit{CRCNS (Collaborative Research in Computational Neuroscience)}\footnote{\url{https://crcns.org}} and \textit{G-Node (German Neuroinformatics Node)}\footnote{\url{https://doid.gin.g-node.org}}. The server provides example SPARQL queries how the available database can be used. The service is free to use and free to contribute, any additions to the database are welcome and encouraged.
...
The following query is similar to the previous example, but adjusted for the Fuseki server. The SPARQL queries work slightly differently in the Java Apache Jena and the Python rdflib implementation.
...
\subsection{Metadata to RDF graph workflow} \label{sec:odml_rdf_workflow}

The presented tools can be used to set up fully automated workflows from data acquisition, metadata storage, export to RDF and import into local or publicly available graph databases. Figure ~\ref{fig:workflowSchema} outlines such potential workflows.

\begin{figure}
\begin{center}
\includegraphics[width=0.90\columnwidth]{figures/workflowSchema.pdf}
\caption{The figure outlines potential metadata workflows from experiment to RDF graph databases.}
...
It can be argued that relational databases are ideally suited for metadata storage and indexing. This argument has its merits, since a dedicated database with a well defined schema (i.e., fixed tables and named columns) allows for targeted, specific queries. Issues arise when reviewing the diversity of metadata that is used to describe scientific experiments even within a single laboratory. If various experiments are described in detail, the resulting metadata will be quite heterogeneous. odML aims at using a general, common data structure while providing the required flexibility to accommodate and store all sorts of heterogeneous metadata. The conversion to RDF is a logical extension of this approach: by converting odML data to RDF and merging the resulting odML documents into a single graph, even heterogeneous metadata are combined and available in a single database. This database can be queried due to the underlying hierarchical odML structure when a comparable relational database could not easily accommodate the required different tables for every individual way experiments were described.
The necessarily complex SPARQL queries are both an advantage and a disadvantage: Searching across such a complex graph will often necessitate multiple steps in query building to arrive at the required result. Users first have to look for hooks into the data; e.g., data sets that contain keywords, authors, specific hardware, are describing stimuli or were using specific scientific models and build more complex queries from there. All of these diverse metadata can be documented and queried via odML RDF but will require a set of query steps to arrive from the initial exploratory queries at a result which links back to the original data sets.
We are aware of this issue. A step to simplify searches has already been done and described by providing a subclassing system to allow easier access to the available data structures and reduce query complexity while keeping the underlying basic data model intact.
Another step further can be the introduction of an abstraction layer that automatically converts a simple keyword based search into useful SPARQL queries. The current Python odML library\footnote{\url{https://github.com/G-Node/python-odml} (RRID:SCR\_001376)} contains a prototype of such an abstraction and is briefly described in the outlook section.

\section{Outlook} \label{sec:outlook}
\subsection{Multiple Endpoint queries} \label{sec:outlook_endpoints}
SPARQL is able to query multiple graphs that are not necessarily part of the same resource. Multiple labs can run their own odML SPARQL servers and all that is required to use and query such resources combined are the URLs where the graphs are available as SPARQL endpoints. A SPARQL query can run over multiple known metadata graphs end return a single result set from various sources.
...
As discussed above and can be found in the odML OWL ontology\footnote{\url{https://github.com/G-Node/python-odml/blob/master/odml/resources/odml-ontology.ttl}}, odML RDF features a set of specific subclasses to reduce the complexity of SPARQL queries. By default odml.Sections with a "type" contained in the subclassing list will automatically be converted to the corresponding odml:Section RDFS subclass. The current implementation of odML RDF further features a mechanism to provide and use a custom dictionary mapping of custom odml types to RDFS subclass names when converting odML to RDF. All definitions will be automatically exported to the RDF file, enabling the usage of custom subclasses in SPARQL queries as well.

In addition to the already available custom subclassing it can be helpful to also add custom RDFS predicates to simplify queries even further. As an example a subclass \texttt{odml:Subject} would be connected to its parent via a \texttt{odml:hasSubject} predicate that would be a subpredicate of \texttt{odml:hasSection}. Thus an \texttt{odml:Subject} instance could be found in a SPARQL query by using both \texttt{odml:hasSubject} and \texttt{odml:hasSection} predicates reducing the complexity of SPARQL queries even further.

\subsection{User friendly query interface}\label{sec:outlook_fuzzy_queries}
As discussed, querying an odML RDF graph can become quite complex. To reduce the complexity, the current version of the python odML library\footnote{\url{https://github.com/G-Node/python-odml} (RRID:SCR\_001376)} contains a prototype to execute "fuzzy queries". These "fuzzy queries" take provided keywords and map these keywords automatically to SPARQL queries aimed at the main odML entities "Document", "Section" and "Property". The results of these subqueries are collected and presented to the user. This way, users do not have to be familiar with the specific structure of each data set and will also not require any knowledge in writing their own SPARQL queries. Based on this prototype the "fuzzy queries" could be expanded to always include links to the original data sets when available as well as the option to expand on useful further queries based on the current result set.

\subsection{Automated graph updates}\label{sec:outlook_gin_integration}
As mentioned, G-Node provides a public resource\footnote{\url{https://meta.g-node.org}} where published data sets from CRCNS and G-Node can be queried. This resource is open for use and also open to contribute. Currently any new addition has to be sent to the G-Node and will be manually added to the graph. A plan for automation is a dedicated microservice that is able to upload valid odML RDF files to the metadata query server. This microservice can be hooked up to G-Nodes' data hosting server, GIN\footnote{\url{https://gin.g-node.org} (G-Node Data Infrastructure Services, RRID:SCR\_015864)}. Any odML files uploaded to this service can then be automatically converted to RDF on demand and uploaded to the common public graph. This would greatly decrease the workload and time required when users are publishing and want to make their data sets findable and accessible.
