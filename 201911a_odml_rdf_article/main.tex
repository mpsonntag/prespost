...
\begin{abstract}
Annotation of research data with metadata is crucial to provide context for analysis and  re-use. The odML format offers a flexible and comprehensive solution for the scientist to collect and organize metadata in a structured form that is both human readable and machine actionable for documentation and automated analysis. To further support the FAIR principles, we present tools to export metadata from odML to \textit{RDF}\footnote{\url{https://www.w3.org/TR/2014/REC-rdf11-concepts-20140225}}, which opens any metadata to \textit{Semantic Web}\footnote{\url{https://www.w3.org/standards/semanticweb}} services. The open source G-Node SPARQL server is aimed at providing searchable whole metadata sets for meta analyses and also providing links to the actual published scientific data set. Scientists can upload their metadata to make their data find-able and accessible even if it was a data publication or if it is an unpublished data set.
\end{abstract}
...
\begin{figure}
  \centering
  \subfloat[odML data model]{
    \includegraphics[width=.6\linewidth, height=4cm, keepaspectratio, valign=T]{figures/figOdmlModelA.pdf}
    \label{fig:odmlModelData}
  }
  \subfloat[odML structure]{
    \includegraphics[width=.35\linewidth, height=4cm, keepaspectratio, valign=T]{figures/figOdmlModelB.pdf}
    \vphantom{\includegraphics[width=0.35\textwidth, height=4cm, valign=T]{figures/figOdmlModelA.pdf}}
...
%[MS] Move to another part or remove?
...
%[MS] Not sure if we need to show how to load RDF files into a local graph for completeness sake.
%[MS] If required support minimal example files as well.

Code listing ~\ref{lst:create_graph} shows how to create an in-memory RDF graph from two example odML RDF files.

\begin{lstlisting}[label=lst:create_graph, caption=Create RDF graph]
...
The following query uses the graph created in the above example.

\begin{lstlisting}[label=lst:query_graph, caption=Example query]
from rdflib import RDF, RDFS, Namespace
from rdflib.plugins.sparql import prepareQuery

# Providing the namespaces that will be used in the query
ns_dict = {"odml": Namespace("https://g-node.org/odml-rdf#"), "rdf":RDF, "rdfs":RDFS}
...
WHERE {
  ...
  ?idtype ?predA ?id_type_value .
  ?doival ?predB ?doi_val .
  ...
  ?p odml:hasValue ?v .
  {?v ?predC "Neuroscience"} UNION {?v ?predC "Electrophysiology"} .
  ?v ?predC ?keyword .
  FILTER regex(str(?predA), '_')
  FILTER regex(str(?predB), '_')
}"""
prep_query = prepareQuery(query, initNs=ns_dict)

# Printing the results
for row in local_graph.query(prep_query):
    print("%s %s %s %s" % (row.file, row.keyword, row.id_type_value, row.doi_val))
...
\begin{lstlisting}[label=lst:query_graph_subclass, caption=Subclass query example]
query = """
SELECT DISTINCT * WHERE {
  ...
  ?sec_id odml:hasName ?sec_id_name .
  ?sec_id a odml:Identifier .
...
This query will lead to the following results:

\begin{lstlisting}[label=lst:query_graph_subclass_res, caption=Subclass query result]
...
\begin{lstlisting}[label=lst:query_server, caption=Example query server]
...
% [MS] Is a suggested workflow something we want to add?
...
% [MS] probably need to compare this to e.g. datajoint\footnote{\url{https://datajoint.io}}?
...
\begin{lstlisting}[label=lst:multi_graph, caption=Abstract multiple graph query]
